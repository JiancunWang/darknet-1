        ...
        float C[FIXED_SIZE], int ldc)
{
    ...
    float *IFMAP;
    IFMAP=(float *) sds_alloc(27*600*sizeof(float));

    float *OFMAP;
    OFMAP=(float *) sds_alloc(16*600*sizeof(float));

    float *OFMAP_TMP;
    OFMAP_TMP=(float *) sds_alloc(16*600*sizeof(float));

    if (!IFMAP || !OFMAP || !OFMAP_TMP ) {
       if (IFMAP)     sds_free(IFMAP);
       if (OFMAP)     sds_free(OFMAP);
       if (OFMAP_TMP) sds_free(OFMAP_TMP);
